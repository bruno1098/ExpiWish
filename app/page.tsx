...
const InteractiveModal = ({ title, isOpen, onClose, selectedItem, allFeedbacks }: InteractiveModalProps) => {
  ...
  // Primeiro aplicar filtro para remover feedbacks "Não identificados"
  const validFeedbacks = filterValidFeedbacks(allFeedbacks);
  
  // Filtrar feedbacks com base no item selecionado
  const filteredFeedbacks = validFeedbacks.filter(feedback => {
    ...
    switch (selectedItem.type) {
      ...
      case 'sector':
        ...
        const feedbackSectors = feedback.sector ? feedback.sector.toLowerCase().split(';').map((s: string) => s.trim()) : [];
        const matchesSector = feedbackSectors.some((s: string) => s === sectorValue);
      ...
      case 'problem':
        ...
        const feedbackProblems = feedback.problem ? feedback.problem.toLowerCase().split(';').map((p: string) => p.trim()) : [];
        const matchesProblem = feedbackProblems.some((p: string) => p.includes(problemValue));
      ...
      case 'keyword':
        ...
        const feedbackKeywords = feedback.keyword ? feedback.keyword.toLowerCase().split(';').map((k: string) => k.trim()) : [];
        const matchesKeyword = feedbackKeywords.some((k: string) => k === keywordValue || k.includes(keywordValue));
...
function DashboardContent() {
  ...
  const filteredData = useMemo(() => {
    ...
    const hotelFilteredFeedbacks = allFeedbacks.filter(
      feedback => feedback.hotel === selectedHotel || feedback.source === selectedHotel
    );
    
    // Aplicar filtro para remover feedbacks "Não identificados"
    const filteredFeedbacks = filterValidFeedbacks(hotelFilteredFeedbacks);
      ...
      if (feedback.keyword) {
        keywordCounts[feedback.keyword] = (keywordCounts[feedback.keyword] || 0) + 1;
          ...
          problems.forEach((problem: string) => {
            ...
            const isValidProblemLocal = (p: string) => {
            ...
            if (isValidProblemLocal(problem)) {
          ...
          <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
            ...
            <Card className="p-4 md:p-6 hover:shadow-lg transition-shadow relative">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium">Distribuição por Departamento</h3>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => handleOpenDetailedChart('sector', 'Distribuição por Departamento', filteredData.sectorDistribution)}
              ...
              <div className="h-[300px] relative">
                {hasData ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={filteredData.sectorDistribution.slice(0, 6)}
                        dataKey="count"
                        nameKey="sector"
                        cx="50%"
                        cy="50%"
                        ...
                        className="cursor-pointer"
                      >
                        {filteredData.sectorDistribution.slice(0, 6).map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                        ))}
                      </Pie>
                      <RechartsTooltip content={<CustomTooltip />} />
