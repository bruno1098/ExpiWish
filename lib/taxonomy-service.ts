...
// Configuração padrão - OTIMIZADA PARA PRECISÃO
const DEFAULT_CONFIG: TaxonomyConfig = {
  embedding_model: 'text-embedding-3-small',
  similarity_threshold: 0.85,
  recall_top_n: 6,  // ✅ REDUZIDO: Menos candidatos = mais precisão
  min_confidence_threshold: 0.6,  // ✅ AUMENTADO: Só aceitar matches mais confiantes
...
export async function findCandidates(
  ...
  for (const keyword of taxonomy.keywords) {
    try {
      ...
      // ✅ THRESHOLD AJUSTADO: 0.40 para keywords (balanceado)
      if (similarity > 0.40) { 
  ...
  for (const problem of taxonomy.problems) {
    try {
      ...
      // ✅ THRESHOLD AUMENTADO: 0.2 → 0.55 para problems (ainda mais rigoroso)
      if (similarity > 0.55) { 
  ...
  console.log('📊 Candidatos encontrados:', {
    ...
    threshold_used: { keywords: 0.40, problems: 0.55 },
  ...
  if (finalKeywords.length < 3) {
    console.warn(`⚠️ Apenas ${finalKeywords.length} keywords passaram no threshold 0.40. Aplicando fallback com threshold 0.30...`);
    ...
    for (const keyword of taxonomy.keywords) {
      ...
      const similarity = cosineSimilarity(textEmbedding, keyword.embedding);
      if (similarity > 0.30 && similarity <= 0.40) { // Pegar o que ficou entre 0.30 e 0.40
  ...
  if (finalProblems.length < 3) {
    console.warn(`⚠️ Apenas ${finalProblems.length} problems passaram no threshold 0.55. Aplicando fallback com threshold 0.40...`);
...
export async function createKeyword(
  ...
  // Gerar embedding com contexto rico
  // ✅ MELHORIA: incluir descrição e departamento para contexto semântico
  const contextParts = [
    data.label,
    data.description || '',
...
export async function createProblem(
  ...
  // Gerar embedding com contexto rico
  // ✅ MELHORIA: incluir descrição e departamentos aplicáveis para contexto semântico
  const contextParts = [
    data.label,
    data.description || '',
